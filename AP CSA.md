...
### <a id="dic_primitiveType">Primitive type</a>
...
对于已经声明的变量，在使用过程中，不再需要说明他的数据类型。但是需要保证计算过程是符合规范的。在这里我们对相同的int类型进行数学操作，加法`+`和乘法`*`，其他的数学逻辑也都有相对应<a id="dic_operator">操作符（Operator）</a>
